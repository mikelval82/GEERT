...
from data.ALFONSO.peliculas.cienmetros import labels_metadata

labels_byTrial = np.array( labels_metadata.labels['valence'] )
times = labels_metadata.labels['times']



path_training = './data/' + SUBJECT + '/peliculas/' + film + '/'+ film + '_trial_' + str(1) + '.edf'
raw_train = read_raw_edf(path_training)
EEG_data = raw_train.get_data()
print(EEG_data.shape)
print(labels_byTrial.shape)
#%%
...
for instant in times:
    ini = anterior + 10*fs
    end = int(instant) - 5*fs
    print((end - ini)/fs)
#    print( EEG_data[:,ini:end].shape )
#    EEG_data[-1,ini:end] = EEG_data[-2,ini:end]
    EEG_trials.append( EEG_data[:,ini:end] )
    anterior = end
    
#    plt.figure()
#    for i in range(8):
#        plt.plot(zscore(EEG_data[i,ini:end])/8 + i)
...
step = fs*2# seconds of sliding step * fs
fs = 250
seconds = 6
sample_length = fs*seconds
...
##%% SAVE DATA
np.save('./data/' + SUBJECT + '/peliculas/' + film + '/features_smooth.npy', features_smooth)
np.save('./data/' + SUBJECT + '/peliculas/' + film + '/labels.npy', labels)
np.save('./data/' + SUBJECT + '/peliculas/' + film + '/trials_labels_map.npy', trials_labels_map)
#%% LOAD DATA
import numpy as np
#SUBJECT = 'LIDIA'
#film = 'wonder'
features_smooth = np.load('./data/' + SUBJECT + '/peliculas/' + film + '/features_smooth.npy')
labels = np.load('./data/' + SUBJECT + '/peliculas/' + film + '/labels.npy')
trials_labels_map = np.load('./data/' + SUBJECT + '/peliculas/' + film + '/trials_labels_map.npy')
...
for i in check:
    ...
    for indx in range(10):
        ...
        X_train,y_train,X_test,y_test = get_aleatory_k_trials_out(all_features,all_labels,emotions,old_indx,size=2)
        select_features = i
